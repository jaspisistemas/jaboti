...
class MessagesGridApp {
  ...
  setupFloatingAudioControl() {
    ...
    document.addEventListener('keydown', (e) => {
      if (
        ...
        this.floatingControl.classList.contains('show')
      ) {
        this.closeFloatingControl();
      }
    });
  ...
  updateNewMessagesCount() {
    const text =
      this.pendingMessages === 1 ? '1 nova mensagem' : `${this.pendingMessages} novas mensagens`;
  ...
  renderAttachment(attachment) {
    ...
    switch (attachment.type) {
      ...
      case 'AUDIO':
        ...
        this.setupAudioControls(attachmentDiv.querySelector('audio'), attachmentDiv);
        break;
      case 'DOCUMENT':
      case 'FILE':
      case 'DOCUMENTO':
  ...
  updateMessageStatus(id, status) {
    ...
    const messageDiv = this.messagesContainer.querySelector(`.message[data-id="${id}"]`);
    if (!messageDiv) {
  ...
  _bindControlButtons() {
    ...
    this.overlay.addEventListener('click', (e) => {
      if (
        ...
        e.target === document.getElementById('image-viewer-container')
      ) {
        this.fechar();
      }
    });
