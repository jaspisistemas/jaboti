...
import type { MessageAttachmentType, MessageItem } from '../../vite-env';
...
import { addMessage, removeMessage, setChatMessages, updateMessage } from './slices/messagesSlice';

// Extender a interface Window para incluir audioManager
declare global {
  interface Window {
    audioManager?: ReturnType<typeof useAudioManager>;
    setCurrentAudioElement?: (audio: HTMLAudioElement | null, messageId: string | null) => void;
  }
}
...
const MediaPreviewModal = memo(function MediaPreviewModal({
  ...
  const scrollToActiveThumbnail = useCallback(() => {
    ...
    if (!container || !mediaItems || selectedIndex < 0 || selectedIndex >= mediaItems.length)
      return;
    ...
    if (activeThumbnail) {
      ...
      if (!isVisible) {
        ...
        const scrollLeft =
          activeThumbnail.offsetLeft - container.clientWidth / 2 + activeThumbnail.clientWidth / 2;
    ...
    <div className="media-preview-modal" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="media-preview-container">
        ...
        <div className="media-preview-header">
          <h3 className="media-preview-title">Enviar mídia</h3>
          <button className="media-preview-close" onClick={onClose} type="button" title="Fechar">
        ...
        <div className="media-preview-body">
          ...
          {currentMedia.type === 'document' ? (
            <div className={`main-document-container ${darkBackground ? 'dark-bg' : 'light-bg'}`}>
              ...
              <div className="document-filename">{currentMedia.file.name}</div>
            </div>
          ) : (
            <div className={`main-media-container ${darkBackground ? 'dark-bg' : 'light-bg'}`}>
              {currentMedia.type === 'video' ? (
                <video className="main-media" src={currentMedia.url} controls preload="metadata" />
              ) : currentMedia.type === 'audio' ? (
                <audio className="main-media" src={currentMedia.url} controls preload="metadata" />
              ) : (
                <img className="main-media" src={currentMedia.url} alt="Preview" />
              )}
            </div>
          )}
          ...
          <div className="message-input-container">
            <label className="message-input-label" htmlFor="media-message">
              {currentMedia.type === 'document'
                ? 'Legenda para este arquivo (opcional)'
                : 'Mensagem para este arquivo (opcional)'}
              <span className="input-hint">
                Enter para enviar • Shift+Enter para nova linha • Alt+←/→ para navegar • Alt+Del
                para remover • Ctrl+V para colar imagem • Ctrl+Alt+←/→ para reordenar
              </span>
            </label>
            <textarea
              ...
              placeholder={
                currentMedia.type === 'document'
                  ? 'Digite uma legenda para este arquivo...'
                  : 'Digite uma mensagem para este arquivo...'
              }
                ...
                {mediaItems.map((item, index) => (
                  ...
                  >
                    {item.type === 'document' ? (
                      ...
                      <video className="thumbnail-media" src={item.url} preload="metadata" />
                    ) : item.type === 'audio' ? (
                      <div className="thumbnail-document">
                      ...
                      <img className="thumbnail-media" src={item.url} alt="Thumbnail" />
                    )}
                    <button
...
const MediaMessage = memo(function MediaMessage({
  ...
  return (
    <div className={`message ${isOwn ? 'sent' : 'received'}`} data-message-id={message.id}>
      <div className="message-content">
        <div className="message-bubble">
          ...
          {isVideo ? (
            ...
            <div className="audio-attachment">
              <AudioMessage url={attachment.url} messageId={message.id} />
            </div>
          ) : (
...
export function ChatMessages() {
  ...
  // Sistema de controle global de áudio (apenas um por vez)
  const audioManager = useAudioManager();

  // Debug: Log quando selectedId muda

  // Configurar container de mensagens no controle de áudio
  useEffect(() => {
    if (containerRef.current) {
      // Expor o gerenciador de áudio globalmente
      (window as any).audioManager = audioManager;

      // Expor função para definir áudio ativo
      window.setCurrentAudioElement = (
        audio: HTMLAudioElement | null,
        messageId: string | null
      ) => {
        setCurrentAudioElement(audio);
        setCurrentAudioMessageId(messageId);
        setFloatingAudioVisible(false); // Resetar visibilidade
      };
    }
  }, [audioManager]);
  ...
  // Estado para o FloatingAudioControl
  const [floatingAudioVisible, setFloatingAudioVisible] = useState(false);
  ...
  const [closeWithMessage, setCloseWithMessage] = useState<
    'nenhuma' | 'porAtendente' | 'porInatividade'
  >('nenhuma');
  ...
  const handleSendMediaItems = useCallback(
    async (mediaItems: MediaPreviewItem[]) => {
  ...
  const handleSendAudioMessage = useCallback(
    async (audioBlob: Blob) => {
      ...
      dispatch(addMessage({ chatId: selectedId, message: tempMessage }));
      dispatch(
        atualizarPreview({ chatId: selectedId, ultimaMensagem: 'Áudio gravado', horario: hora })
      );
  ...
  useEffect(() => {
    ...
    if (transferOpen && depId) {
      api
        .get(`/departamentos/${depId}/membros`)
        .then(({ data }) => {
          const list = Array.isArray(data) ? data : [];
          setAttendants(
            list.map((p: any) => ({ id: String(p.id), name: p.name || p.nome || `#${p.id}` }))
          );
        })
        .catch(() => setAttendants([]));
    }
  ...
  useEffect(() => {
    ...
    api
      .get(`/atendimentos/${selectedId}/messages`, { params: { limit: 100 } })
      .then(({ data }) => {
        ...
        const formattedMessages: MessageItem[] = messagesList.map((msg: any) => {
          ...
          if (
            ...
            msg.content &&
            msg.content.trim() &&
            invalidDefaultValues.includes(msg.content.trim())
          ) {
            ...
            console.error(
              '🔧 LOCALIZAÇÃO: Verificar endpoint POST /atendimentos/message no backend'
            );
            console.groupEnd();
            ...
            if (!sessionStorage.getItem(alertKey)) {
              ...
              setTimeout(() => {
                console.warn(
                  `🚨 ALERTA: Backend retornando valor inválido "${msg.content}" para mídia (ID: ${msg.id})`
                );
              }, 100);
            }
        ...
        if (messagesList.length > 0) {
          const unreadMessages = messagesList.filter(
            (msg: any) => msg.senderType !== 'ATTENDANT' && !msg.readAt
          );

          if (unreadMessages.length > 0) {
            // Marcar todas como lidas em lote
            api
              .patch(`/atendimentos/${selectedId}/messages/read-bulk`)
  ...
  const onScroll = useCallback(() => {
    ...
    // Verificar se deve mostrar a barra flutuante de áudio
    if (currentAudioElement && containerRef.current) {
      const containerRect = containerRef.current.getBoundingClientRect();
      const audioMessage = document.querySelector(`[data-message-id="${currentAudioMessageId}"]`);

      if (audioMessage) {
        const audioRect = audioMessage.getBoundingClientRect();
        const isAudioVisible =
          audioRect.top >= containerRect.top && audioRect.bottom <= containerRect.bottom;

        setFloatingAudioVisible(!isAudioVisible);
      }
    }
  }, [isNearBottom, currentAudioElement, currentAudioMessageId]);
  ...
  // Funções para o FloatingAudioControl
  const handleLocateAudio = useCallback(() => {
    if (currentAudioMessageId && containerRef.current) {
      const audioMessage = document.querySelector(`[data-message-id="${currentAudioMessageId}"]`);
      if (audioMessage) {
        audioMessage.scrollIntoView({ behavior: 'smooth', block: 'center' });
        setFloatingAudioVisible(false);
      }
    }
  ...
  return (
    <div className="messages-grid-container">
          ...
          <div className="header-left">
            ...
            <div className="header-info">
              <div className="header-name-row">
                ...
                {chat.status && (
                  <span className={`header-status status-${chat.status}`}>{chat.status}</span>
                )}
              </div>
      ...
      <div className="messages-container" id="messagesContainer" ref={containerRef}>
        {chat && (
          ...
          >
            ...
            {pending === 1 ? '1 nova mensagem' : `${pending} novas mensagens`}
          </div>
        )}
        {chat && (
          <button
            className="scroll-to-bottom-btn"
            style={{ display: isNearBottom() ? 'none' : 'flex' }}
            onClick={scrollToBottom}
          ...
          grouped.map((item) => {
            ...
            if (item.attachment) {
              ...
              if (
                !item.attachment.url ||
                item.attachment.url === '' ||
                item.attachment.url === 'undefined' ||
                item.attachment.url === 'null'
              ) {
                return (
                  <div
                    key={item.id}
                    className={`message ${!!item.senderIsMe ? 'sent' : 'received'} error-message`}
                  >
                    <div className="message-content">
        ...
        >
          <div className="modal-card">
            <div className="modal-header">
              ...
              <button type="button" className="modal-close" onClick={() => setCloseOpen(false)}>
            ...
            <div className="modal-body">
              ...
              <div className="form-field">
                ...
                <select
                  value={closeWithMessage}
                  onChange={(e) => setCloseWithMessage(e.target.value as any)}
                >
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    const hora = `${pad(now.getHours())}:${pad(now.getMinutes())}`;
                    if (content) {
                      await api.post('/atendimentos/message', {
                        atendimentoId: idNum,
                        content,
                        senderType: 'ATTENDANT',
                      });
        ...
        >
          <div className="modal-card">
            <div className="modal-header">
              ...
              <button type="button" className="modal-close" onClick={() => setTransferOpen(false)}>
                <i className="fas fa-times" />
              </button>
            </div>
            <div className="modal-body">
              ...
              <div className="form-field" style={{ alignItems: 'flex-start' }}>
                ...
                <div>
                  <label style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <input
                      type="checkbox"
                      checked={transferGiveMessage}
                      onChange={(e) => setTransferGiveMessage(e.target.checked)}
                    />
            ...
            <div className="modal-footer">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  if (!depId && !attId) {
                    setTransferError('Informe ao menos um: departamento ou atendente.');
                    return;
                  }
                  try {
                    await api.patch(`/atendimentos/${idNum}/transfer`, {
                    ...
                    if (transferGiveMessage) {
                      const depName = depId
                        ? departments.find((d) => String(d.id) === String(depId))?.name ||
                          `#${depId}`
                        : '';
                      const attName = attId
                        ? attendants.find((a) => String(a.id) === String(attId))?.name ||
                          `#${attId}`
                        : '';
                      let content = '';
                      if (depId && attId)
                        content = `Atendimento transferido para o departamento ${depName} e para o ${attName}`;
                      else if (depId)
                        content = `Atendimento transferido para o departamento ${depName}`;
                    ...
                    setTransferError(
                      e?.response?.data?.message || e?.message || 'Falha ao transferir'
                    );
                  }
                }}
              >
      ...
      {chat && !canInteract && chat.status?.toLowerCase() !== 'finalizado' && (
        <div
          className="composer-bar disabled"
          style={{
              ...
              {attachOpen && (
                <div className="attach-popover">
                  ...
                  <button
                    ...
                    className="attach-item"
                    onClick={(e) => {
                      console.log('Botão Foto, vídeo e áudio clicado');
                      e.preventDefault();
                      e.stopPropagation();
                      setAttachOpen(false);
                  ...
                  >
                    <i className="fas fa-image" /> Foto, vídeo e áudio
                  </button>
                </div>
              )}
              <input
                ref={docInputRef}
                type="file"
                accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,application/*"
                multiple
                style={{ display: 'none' }}
                onChange={(e) => {
            ...
            onPaste={(e) => {
              ...
              for (let i = 0; i < items.length; i++) {
                ...
                if (item.type.startsWith('image/')) {
                  ...
                  if (file) {
                    console.log(
                      'Imagem colada no input principal:',
                      file.name,
                      file.type,
                      file.size
                    );
      ...
      {/* Barra flutuante de controle de áudio */}
      <FloatingAudioControl
        isVisible={floatingAudioVisible}
        audioElement={currentAudioElement}
        onClose={handleCloseFloatingAudio}
        onLocate={handleLocateAudio}
      />
    </div>
  );
}
